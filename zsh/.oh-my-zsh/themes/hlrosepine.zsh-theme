...
if [[ $TERM = (*256color|*rxvt*) ]]; then
 ...
 # hotpink="%{${(%):-"%F{161}"}%}"
 # limegreen="%{${(%):-"%F{118}"}%}"
  turquoise="%{${(%):-"%F{67}"}%}"       # Approximate to thm_pine (#3e8fb0)
  orange="%{${(%):-"%F{179}"}%}"         # Approximate to thm_gold (#f6c177)
  purple="%{${(%):-"%F{183}"}%}"         # Approximate to thm_iris (#c4a7e7)
  hotpink="%{${(%):-"%F{168}"}%}"        # Approximate to thm_love (#eb6f92)
  limegreen="%{${(%):-"%F{152}"}%}"      # Approximate to thm_foam (#9ccfd8)
else
  turquoise="%{${(%):-"%F{cyan}"}%}"
  orange="%{${(%):-"%F{yellow}"}%}"
  purple="%{${(%):-"%F{magenta}"}%}"
